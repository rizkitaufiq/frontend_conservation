...
const Navbar = () => {
    ...
    return (
        <>
                ...
                <div className="text-lg container mx-auto flex justify-between items-center font-medium">
                    <div className="flex space-x-14 items-center">
                        ...
                        <ul className="md:flex space-x-12 hidden">
                            {
                                navItems.map(({ link, path }) => <Link activeClass="active" spy={true} smooth={true} offset={-100} key={link} to={path} className="block hover:text-tertiary cursor-pointer">{link}</Link>)
                            }
                        </ul>
                    </div>
...

            {
                isMenuOpen ?
                    ...
                    <div className='space-y-4 px-4 pt-24 pb-5 bg-secondary block fixed top-0 right-0 left-0 md:hidden'>
                        {
                            navItems.map(({ link, path }) => <Link activeClass="active" spy={true} smooth={true} offset={-75}
                                key={link} to={path} className="text-white block hover:text-gray-300 cursor-pointer"
                                onClick={toogleMenu}>{link}</Link>)
                        }
                    </div>
            ...
            {/* <div className={'space-y-4 px-4 pt-5 pb-5 bg-quaternary ${isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden"}  '}>
                {
                    navItems.map(({link,path}) => <a key={link} href={path} className="text-black block hover:text-gray-300">{link}</a>)
                }
            </div>  */}
        </>
